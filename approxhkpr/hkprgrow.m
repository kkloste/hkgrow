function [bestset,bestcond,bestcut,bestvol] = hkprgrow(A,vert,tol,alphat,debugflag,varargin)
% HKPRGROW Grow a cluster around a vertex using a heatkernel-pagerank algorithm
%
% [bestset,cond,cut,vol] = hkprgrow(A,vert,tol,alphat,debugflag,varargin) computes exp(alpha*A)(:,vert) (a column) and
...
%
% [bestset,cond,cut,vol] = hkprgrow(A,verts) computes exp(alpha*P)*[verts]
% tries to find a set that is 2 to 300000 times the size of the input group
% of vertices.  
%
% ... hkprgrow(A,verts,'key',value,'key',value) specifies optional arguments
%
...
%   then call hkprgrow(A,verts,'expands',[12 18]).  
...
%
%   new approx_hkpr function used in hkprgrow.m
%   coded from the approx_hkpr algorithm from Olivia Simpson and Fan Chung
...
if debugflag==1,    fprintf('hkprgrow.m: Called hkpr_mex on set of size=%i with expand=%i\n', numel(vert), curexpand); end

%    [curset cond cut vol] = hkpr_mex(A,vert,curexpand,p.Results.alpha, tol);
    [curset cond cut vol] = hkpr_mex(A,vert,curexpand,alphat, tol, debugflag);

if debugflag==1,    fprintf('hkprgrow.m: hkpr_mex done on set of size=%i with expand=%i\n', numel(vert), curexpand); end
