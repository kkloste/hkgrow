function pprgrow_figure(A,ginfo,ncp,cc,showtitle,showlbls)
clf;
...
loglog(cc.size,cc.cond,'-','LineWidth',1.5,'Color',[0.5,0.1,.9]);


minverts = neighborhoodmin(A,ginfo.neigh.cond,0);
minverts = minverts(ginfo.degrees(minverts)>=5);

pprgrow = pprgrowneigh(A,minverts);

[h1,h2] = ncpplot(pprgrow.cond, pprgrow.size);
set(h1,'Visible','off');
...
pprcores = pprgrowcores(A,cc);

best = accumarray(pprcores.size,pprcores.cond,[],@min);
besti = find(best);
loglog(besti,best(besti),'o-','Color',[216,41,0]/255);
