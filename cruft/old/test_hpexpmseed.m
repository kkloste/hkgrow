function [error, time_x] = test_hpexpmseed(filename, path, numnonzeros, tol, cluster_size)
% TEST_HPEXPMSEED   [error, time] = test_hpexpmseed(filename, path,numnonzeros,tol,cluster_size)
...
tic; x = hpexpmseed_mex(P,c,cluster_size,tol); time_x = toc, error = norm(hkpr-x,1)/norm(hkpr,1)
